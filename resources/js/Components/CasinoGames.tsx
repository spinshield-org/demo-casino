...
export default function CasinoGames({ selectedCurrency, games, showOptions }: CasinoGamesProps) {
  ...
  const filterOptions = [
    ...
    { value: "featureBuy", label: "Bonus Buy" },
    { value: "demoMode", label: "Fun Mode" },
  ];
  ...
  function openGame(event, type, slug) {
    if (type === "real") {
      var url = "/launch/" + selectedCurrency + "/" + slug;
    } else {
      var url = "/launch/demo/" + slug;
    }
    window.open(url, "_blank");
    event.preventDefault();
  ...
  function cdnPrefix(s) {
    return showOptions.cdnServers[s];
  }
  ...
  return (
    <div className="py-1">
      <div className="max-w-7xl mx-auto px-2 sm:px-4">
        <Card className="bg-theme-800/50 bg-gradient-to-bl from-theme-700/50 via-transparent rounded-md mb-4">
          <CardHeader>
            <CardTitle></CardTitle>
          </CardHeader>
          <CardContent className="space-y-1">
            <div className="flex items-center space-x-2">
              <Input
                type="text"
                className="min-w-[100px] bg-theme-800 ring-theme-700 ring-offset-theme-600 border-theme-500 text-theme-50 font-semibold rounded-sm"
                placeholder="Search.."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
              <Select onValueChange={setSelectedProvider}>
                <SelectTrigger className="w-[215px] min-w[215px] bg-theme-800 ring-theme-700 ring-offset-theme-500 border-theme-500 text-theme-50 rounded-sm">
                  <SelectValue placeholder="All Providers" />
                </SelectTrigger>
                <SelectContent className="bg-theme-800 ring-theme-700 ring-offset-theme-500 border-theme-500 text-theme-50">
                  <SelectItem value="" key="all">All Providers</SelectItem>
                  {providers.map(provider => (
                    <SelectItem value={provider} key={provider}>{provider}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <div className="hidden sm:flex text-xs items-center space-x-2">
                <Select defaultValue={selectedFilter} onValueChange={setSelectedFilter}>
                  <SelectTrigger className="w-[135px] bg-theme-800 ring-theme-700 ring-offset-theme-500 border-theme-500 text-theme-50 rounded-sm">
                  ...
                  <Fade key={game.s} duration={500}>
                    <Card className="group rounded-lg scale-95 bg-transparant transition-all ease-in-out duration-300 hover:cursor-pointer">
                      <AspectRatio ratio={1 / 1} className="bg-transparant overflow-hidden rounded-lg">
                        <img
                          src={cdnPrefix(game.cdn) + game.s + ".webp"}
                          alt={game.n}
                          sizes="auto"
                      ...
                      <img
                          ...
                          alt={game.n + "-provider"}
                          sizes="auto"
                          className="w-6 h-6 border bg-theme-850 rounded-full border-theme-100 shadow absolute right-1 bottom-1 transition-all group-hover:opacity-25 transition-opacity"
                        />
                      {game.new === true && (
                            <img
                            ...
                            alt={game.n + "-new"}
                            sizes="auto"
                            className="w-7 h-7 m-3- shadow absolute rounded-md border-3 border-[#e31351] right-1 top-2 transition-all group-hover:opacity-50"
                          />
                      )}
                       {recentGames.includes(game.s) ? (
                            ...
                            alt={game.n + "-recent-game"}
                             sizes="auto"
                            className="w-6 h-6 p-1 border bg-theme-900 rounded-full border-theme-50 shadow absolute left-1 bottom-2 transition-all group-hover:opacity-50"
                          />
                      ) : null}
          ...
          <CardFooter className="flex justify-center">
            {filteredGames.length < 1 && (
              <Button className="mb-10 mt-2-" variant="outline" disabled="true">
                  No games to show you :(
              </Button>
            )}
