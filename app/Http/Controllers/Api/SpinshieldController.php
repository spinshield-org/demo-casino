...
class SpinshieldController extends \App\Http\Controllers\Controller
{
  function __construct()
  {
    $this->api_login = config('games.api.api_login');
    $this->api_password = config('games.api.api_password');
    $this->salt_key = config('games.api.salt_key');
    $this->endpoint = config('games.api.endpoint');
  }
  ...
  public function callback(Request $request)
  {
    ...
    if($action === "credit" || $action === "debit") {
      ...
      $key = $request->key;
      $keyRecreated = md5($timestamp.$this->salt_key);
      if($keyRecreated !== $key) {
        return [
          "error" => 2,
          "balance" => 0,
      ...
      \App\Events\BalanceUpdateEvent::dispatch($currencyData, 'personal:#'.$user->id);
    }
    $response = [
      "error" => 0,
      "balance" => (int) $balance,
      ...
      if($user->fs_spins > 0) {
        ...
        if($select_game) {
          if($select_game['fs'] === true) {
            $this->addFreeRounds($username, $password, $game_id, $currency, $user->fs_spins, 0);
            $user->update([
              "fs_available" => 0,
              "fs_spins" => 0,
            ]);
